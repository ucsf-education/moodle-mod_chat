...
$url = new moodle_url('/mod/chat/gui_ajax/index.php', ['id' => $id]);
if ($groupid !== 0) {
    $url->param('groupid', $groupid);
}
$PAGE->set_url($url);
$PAGE->set_popup_notification_allowed(false); // No popup notifications in the chat window.
$PAGE->requires->strings_for_js(['coursetheme', 'bubble', 'compact'], 'mod_chat');

$chat = $DB->get_record('chat', ['id' => $id], '*', MUST_EXIST);
$course = $DB->get_record('course', ['id' => $chat->course], '*', MUST_EXIST);
...
$module = [
    ...
    'requires'  => ['base', 'dom', 'event', 'event-mouseenter', 'event-key', 'json-parse', 'io', 'overlay', 'yui2-resize',
                         'yui2-layout', 'yui2-menu'],
    'strings'   => [['send', 'chat'], ['sending', 'chat'], ['inputarea', 'chat'], ['userlist', 'chat'],
                         ['modulename', 'chat'], ['beep', 'chat'], ['talk', 'chat']],
];
$modulecfg = [
...
$PAGE->requires->js_init_call('M.mod_chat_ajax.init', [$modulecfg], false, $module);
...
echo $OUTPUT->header();
echo $OUTPUT->box(html_writer::tag('h2', get_string('participants'), ['class' => 'accesshide']) .
        '<ul id="users-list" class="list-group"></ul>', '', 'chat-userlist');
echo $OUTPUT->box('', '', 'chat-options');
echo $OUTPUT->box(html_writer::tag('h2', get_string('messages', 'chat'), ['class' => 'accesshide']) .
...
echo $OUTPUT->box(html_writer::tag('h2', get_string('composemessage', 'chat'), ['class' => 'accesshide']) .
