...
if (has_capability('mod/chat:chat', $context)) {
    ...
    echo $OUTPUT->action_link(
        ...
        new popup_action(
            'click',
            $chattarget,
            "chat{$course->id}_{$chat->id}{$groupparam}",
    ...
    echo $OUTPUT->action_link(
        ...
        $action,
        ['title' => get_string('modulename', 'chat'), 'class' => 'btn btn-secondary']
    );
    echo html_writer::end_div();
    echo html_writer::end_div();
    echo html_writer::end_div();
    ...
    if (($chat->studentlogs or has_capability('mod/chat:readlog', $context)) && !$PAGE->has_secondary_navigation()) {
        if ($msg = chat_get_session_messages($chat->id, $currentgroup)) {
            ...
            echo html_writer::link(
                new moodle_url('/mod/chat/report.php', ['id' => $cm->id]),
...
if ($chatusers = chat_get_users($chat->id, $currentgroup, $cm->groupingid)) {
    ...
    foreach ($chatusers as $chatuser) {
        ...
        $url = new moodle_url('/user/view.php', ['id' => $chatuser->id, 'course' => $chat->course]);
