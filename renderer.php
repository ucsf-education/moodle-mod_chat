...
class mod_chat_renderer extends plugin_renderer_base {
    ...
    protected function render_event_message(event_message $eventmessage) {
        ...
        return str_replace($patterns, $replacements, $chatthemecfg->event_message);
    }
    ...
    protected function render_user_message(user_message $usermessage) {
        ...
        if (!empty($chatthemecfg->avatar) and !empty($chatthemecfg->align)) {
            if (!empty($usermessage->mymessageclass)) {
                $output = str_replace($patterns, $replacements, $chatthemecfg->user_message_right);
            } else {
                $output = str_replace($patterns, $replacements, $chatthemecfg->user_message_left);
            }
        } else {
            $output = str_replace($patterns, $replacements, $chatthemecfg->user_message);
        }
